...
import { Plus, Trash2, Printer, ArrowLeft, Save, Calendar, HelpCircle, Download, Eye, EyeOff, FileText, Calculator } from 'lucide-react';
...
export default function KarachiLedgerPage() {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem('karachi-ledger');
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (entries.length > 0) {
      localStorage.setItem('karachi-ledger', JSON.stringify(entries));
    }
  }, [entries]);
  ...
  const addEntry = () => {
    ...
    const newEntry: LedgerEntry = {
      ...
      folio: formData.folio || `F-${entries.length + 1}`,
  ...
  const deleteEntry = (id: string) => {
    if (window.confirm('⚠️ Are you sure you want to delete this entry? This action cannot be undone.')) {
      ...
      if (filtered.length === 0) {
        localStorage.removeItem('karachi-ledger');
      }
    }
  };
  ...
  const handleSave = () => {
    ...
    link.href = url;
    link.download = `karachi-ledger-${currentDate}.json`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    
    alert('✅ Ledger saved and downloaded successfully!');
  };
  ...
  const clearAllEntries = () => {
    if (window.confirm('⚠️ Are you sure you want to delete ALL entries? This cannot be undone!')) {
      setEntries([]);
      localStorage.removeItem('karachi-ledger');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-gray-100 p-3 sm:p-4 md:p-6">
      <div className="max-w-7xl mx-auto space-y-4 md:space-y-6">
        ...
        {/* Header Section - Fixed Text Visibility */}
        <div className="bg-gradient-to-r from-blue-800 to-indigo-900 rounded-2xl shadow-xl p-4 md:p-6 text-white print:rounded-none print:shadow-none">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-blue-100 text-sm mt-1 font-medium">
                      Professional Accounting System • {new Date(currentDate).getFullYear()}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button 
                onClick={handleSave} 
                size="sm" 
                className="print:hidden bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-600 hover:to-green-700 text-white shadow-lg"
          ...
          {/* Help Panel - Fixed Text Visibility */}
          {showHelp && (
            <div className="mt-4 p-5 bg-white/10 rounded-xl backdrop-blur-sm border border-white/20">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-lg font-bold text-white">📚 How to Use This Ledger</h3>
                <Button 
                  size="sm" 
                  variant="ghost" 
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div className="bg-white/5 p-4 rounded-lg">
                  <h4 className="font-bold text-white mb-2 flex items-center gap-2">
                    <div className="bg-blue-500 p-1 rounded">1</div>
                    💡 Quick Tips
                  </h4>
                  <ul className="space-y-2 text-gray-200">
                    ...
                    <li className="flex items-start gap-2">
                      <div className="w-1 h-1 bg-white rounded-full mt-2"></div>
                      <span>Use <kbd className="bg-white/20 px-2 py-0.5 rounded text-xs">Tab</kbd> to navigate fields</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <div className="w-1 h-1 bg-white rounded-full mt-2"></div>
                      <span>Press <kbd className="bg-white/20 px-2 py-0.5 rounded text-xs">Enter</kbd> to save entry</span>
                    </li>
                  </ul>
                </div>
                <div className="bg-white/5 p-4 rounded-lg">
                  ...
                  <ul className="space-y-2 text-gray-200">
                    <li className="flex items-start gap-2">
                      <div className="w-1 h-1 bg-white rounded-full mt-2"></div>
                      <span><strong className="text-red-300">Debit:</strong> Money coming IN (Income)</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <div className="w-1 h-1 bg-white rounded-full mt-2"></div>
                      <span><strong className="text-green-300">Credit:</strong> Money going OUT (Expense)</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <div className="w-1 h-1 bg-white rounded-full mt-2"></div>
                ...
                <div className="bg-white/5 p-4 rounded-lg">
                  <h4 className="font-bold text-white mb-2 flex items-center gap-2">
                    <div className="bg-purple-500 p-1 rounded">3</div>
                    🛠️ Features
                  </h4>
                  <ul className="space-y-2 text-gray-200">
                    <li className="flex items-start gap-2">
          ...
          {/* Date and Stats - Fixed Dark Text */}
        ...
        {/* Add Entry Form - Improved Buttons */}
        <Card className="print:hidden border-0 shadow-xl bg-white">
          <CardHeader className="bg-gradient-to-r from-emerald-600 to-teal-700 text-white p-5 rounded-t-xl">
          ...
          <CardContent className="p-5 md:p-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4" onKeyDown={handleKeyPress}>
              <div className="lg:col-span-1">
                ...
                <Input
                  ...
                  className="mt-2 focus:ring-2 focus:ring-emerald-500 border-gray-300"
                />
              </div>
              <div className="sm:col-span-2 lg:col-span-2">
                ...
                <Input
                  ...
                  placeholder="Example: Cash received from ABC Company"
                  className="mt-2 focus:ring-2 focus:ring-emerald-500 border-gray-300"
                  autoFocus
                />
              </div>
              ...
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">💸 Money Paid</p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mt-6 pt-6 border-t border-gray-200">
              <div className="flex flex-wrap gap-3">
                <Button 
                  onClick={addEntry}
                  className="bg-gradient-to-r from-emerald-600 to-teal-700 hover:from-emerald-700 hover:to-teal-800 text-white shadow-lg px-6 py-2"
                >
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                ...
                <div className="flex bg-gray-100 p-1 rounded-lg border border-gray-300">
                  <Button
                    ...
                    className={`px-3 ${balanceView === 'all' ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-700 hover:bg-gray-200'}`}
                  >
        ...
        {/* Ledger Table - Fixed Text Colors */}
                  ...
                  <tr>
                    <td colSpan={8} className="px-5 py-12 text-center">
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <p className="text-gray-600 font-medium">Loading ledger data...</p>
                      </div>
                    </td>
                  </tr>
        ...
        {entries.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 print:grid-cols-3 print:gap-3">
            <div className="bg-gradient-to-br from-red-50 to-white border-2 border-red-300 rounded-2xl p-6 shadow-lg">
              ...
              <p className="text-sm text-gray-600 font-medium">Total money received into account</p>
            ...
            <div className="bg-gradient-to-br from-green-50 to-white border-2 border-green-300 rounded-2xl p-6 shadow-lg">
              ...
              <p className="text-sm text-gray-600 font-medium">Total money paid out from account</p>
              <div className="mt-4 text-xs text-gray-500">
                {entries.filter(e => parseFloat(e.credit) > 0).length} credit entries
              </div>
            </div>
            
            <div className="bg-gradient-to-br from-blue-50 to-white border-2 border-blue-300 rounded-2xl p-6 shadow-lg">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-gray-900 text-lg">Final Balance</h3>
              ...
              <p className="text-4xl md:text-5xl font-bold text-blue-800 mb-2">Rs. {totals.finalBalance}</p>
              <div className="flex items-center gap-3 mt-3">
                ...
                <span className="text-sm text-gray-600 font-medium">
                  {totals.finalType === 'Dr' ? 'You are owed money' : 'You owe money'}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="text-center text-sm text-gray-600 py-5 print:hidden border-t border-gray-200 mt-6">
          <p className="font-medium mb-2">💡 <strong>Important:</strong> This ledger automatically saves to your browser's local storage.</p>
          <p className="text-xs">Use the "Save Backup" button to download a permanent copy. Clearing browser data will delete local entries.</p>
        </div>
      ...
      <style jsx global>{`
        @media print {
          ...
          .text-white, .text-blue-100, .text-gray-300 {
